...
def test_convex_hull_visualization():
    ...
    shape = Shape.from_multipoint(points)

    # Create visualization
    image_mock = np.zeros((30, 30, 3))
    hull_shape = Shape(geometry=shape.convex_hull[0])
    pixels_within = hull_shape.get_pixels_within_convex_hull()[0]
    display_image_with_areas(image_mock, pixels_within, color="red")


def test_get_pixels_within_convex_hull():
    points = Pixels.from_iterable([(0, 0), (2, 0), (2, 2), (0, 2)])
    shape = Shape.from_polygon(points)

    pixels = shape.get_pixels_within_convex_hull()
    assert isinstance(pixels, list)
    assert len(pixels) == 1  # One Pixels object for the single polygon
    assert all(isinstance(p, Pixels) for p in pixels)

    # All pixels should be within the shape bounds
    bounds = shape.bounds.iloc[0]
    for pixel in pixels[0].to_list():
        assert bounds.minx <= pixel[0] <= bounds.maxx
        assert bounds.miny <= pixel[1] <= bounds.maxy


def test_rectangular_shape_convex_hull():
    shape = Shape.from_rectangle(10, 21, 12, 23)
    pixels = shape.get_pixels_within_convex_hull()[0]
    expected_points = [[u, v] for u in range(10, 13) for v in range(21, 24)]
    assert pixels.as_type(int).to_list() == expected_points


def test_point_shape_convex_hull():
    points = [[10, 15], [12, 23]]
    shape = Shape.from_multipoint(Pixels.from_iterable(points))
    pixels = shape.get_pixels_within_convex_hull()[0]
    assert pixels.as_type(int).to_list() == points


def test_triangle_shape_convex_hull():
...
def test_get_pixels_within_convex_hull_resolution():
    shape = Shape.from_rectangle(0, 0, 3, 3)

    pixels_res_1 = shape.get_pixels_within_convex_hull(resolution=1.0)[0]
    pixels_res_2 = shape.get_pixels_within_convex_hull(resolution=2.0)[0]
    pixels_res_05 = shape.get_pixels_within_convex_hull(resolution=0.5)[0]

    # Check expected number of points for each resolution
    assert len(pixels_res_1) == 16  # 4x4 grid
    assert len(pixels_res_2) == 4  # 2x2 grid
    assert len(pixels_res_05) == 49  # 7x7 grid


def test_get_pixels_within_convex_hull_edge_cases():
    # Test single point
    point_shape = Shape.from_point(1.5, 1.5)
    point_pixels = point_shape.get_pixels_within_convex_hull(resolution=1.0)[0]
    assert len(point_pixels) == 1
    assert point_pixels.to_list()[0] == [1.5, 1.5]

    # Test multiple points with same coordinates
    multi_point = Shape.from_multipoint(Pixels.from_iterable([(1.0, 1.0), (1.0, 1.0)]))
    multi_pixels = multi_point.get_pixels_within_convex_hull(resolution=1.0)[0]
    assert multi_pixels.to_list() == [[1.0, 1.0], [1.0, 1.0]]

    # Test line (degenerate case)
    line_shape = Shape.from_line(Pixels.from_iterable([(0, 0), (2, 0)]))
    line_pixels = line_shape.get_pixels_within_convex_hull(resolution=1.0)[0]
    assert line_pixels.to_list() == [[0, 0], [1, 0], [2, 0]]


def test_get_pixels_within_convex_hull_invalid_inputs():
    shape = Shape.from_rectangle(0, 0, 1, 1)

    with pytest.raises(InvalidInputError):
        shape.get_pixels_within_convex_hull(resolution=-1.0)

    with pytest.raises(InvalidInputError):
        shape.get_pixels_within_convex_hull(resolution=0.0)


def test_get_pixels_within_convex_hull_complex_shape():
    # Create an L-shaped polygon
    points = Pixels.from_iterable([(0, 0), (2, 0), (2, 1), (1, 1), (1, 2), (0, 2)])
    shape = Shape.from_polygon(points)

    pixels = shape.get_pixels_within_convex_hull(resolution=1.0)[0]
...
def test_get_pixels_within_convex_hull_with_holes():
    # Create a polygon with a hole
    exterior = Pixels.from_iterable([(0, 0), (3, 0), (3, 3), (0, 3)])
    hole = Pixels.from_iterable([(1, 1), (2, 1), (2, 2), (1, 2)])
    shape = Shape.from_polygon(exterior, holes=[hole])

    pixels = shape.get_pixels_within_convex_hull(resolution=1.0)[0]

    # The convex hull should ignore the hole and return all points
    assert len(pixels) == 16  # 4x4 grid
