...
def test_get_pixels_within_convex_hull():
    ...
    pixels_high_res = shape.get_pixels_within_convex_hull(resolution=0.5)
    pixels_low_res = shape.get_pixels_within_convex_hull(resolution=2.0)

    # Higher resolution should have more points
    assert len(pixels_high_res[0]) > len(pixels_low_res[0])
