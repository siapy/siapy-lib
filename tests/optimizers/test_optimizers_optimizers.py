...
def test_tabular_optimizer_from_tabular_dataset_data_invalid(
    spectral_tabular_dataset,
):
    data = spectral_tabular_dataset.dataset_data
    model = SVR()
    configs = TabularOptimizerConfig()
    with pytest.raises(InvalidInputError, match="Target data is required for optimization"):
        TabularOptimizer.from_tabular_dataset_data(model=model, configs=configs, data=data)
...
def test_tabular_optimizer_with_validation_data_without_targets(
    ...
    with pytest.raises(InvalidInputError, match="validation targets"):
        TabularOptimizer.from_tabular_dataset_data(model=model, configs=configs, data=data, data_val=data_val)
...
def test_tabular_optimizer_get_best_model_no_best_trial(
    ...
    configs = TabularOptimizerConfig()
    optimizer = TabularOptimizer.from_tabular_dataset_data(model=model, configs=configs, data=data)
    with pytest.raises(InvalidInputError, match="Study is not available for model refitting."):
