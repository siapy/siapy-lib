...
def get_number_of_slices(indices_and_filenames):
    ...
    for image_idx in images_indices_unique:
        all_indices_img = indices_and_filenames[indices_and_filenames.images_indices == image_idx]
        objects_indices_unique = all_indices_img.objects_indices.unique()
        for object_idx in objects_indices_unique:
            all_indices_obj = all_indices_img[all_indices_img.objects_indices == object_idx].reset_index(drop=True)
    ...
    return pd.DataFrame(data=slices_num, columns=["images_indices", "objects_indices", "slices_len", "filepaths"])
...
def main(cfg):
    ...
    indices_cam1 = [parse_indices(filename, labels_pd, labels_d) for filename in filenames_cam1]
    indices_cam2 = [parse_indices(filename, labels_pd, labels_d) for filename in filenames_cam2]
    ...
    if cfg.data_loader.labels_of_groups_file_name is not None:
        ...
        labels_file = load_data(cfg, cfg.data_loader.labels_of_groups_file_name,
                                loader="df_xlsx", dir_abs_path=cfg.data_loader.data_dir_path)

        # extract labels from filepaths
        ref_panel = bool(cfg.preparator.reflectance_panel)
        # camera 1
        # extract labels from filepath
        labels_all = slices_num_cam1.apply(lambda row: row.filepaths.split("__")[0].split("_"), axis=1)
        slices_num_cam1["labels_all"] = labels_all
        slices_num_cam1["labels"] = slices_num_cam1.apply(lambda row: parse_labels(row, ref_panel), axis=1)
        ...
        if not slices_num_cam2.empty:
            labels_all = slices_num_cam2.apply(lambda row: row.filepaths.split("__")[0].split("_"), axis=1)
            slices_num_cam2["labels_all"] = labels_all
            slices_num_cam2["labels"] = slices_num_cam2.apply(lambda row: parse_labels(row, ref_panel), axis=1)
            slices_num_cam2 = slices_num_cam2.merge(labels_file, on="labels", how="left")
        ...
        for group in groups_unique:
            slices_num_group = slices_num_cam1[slices_num_cam1.groups == group] \
                                    .sort_values(by="labels").reset_index(drop=True)
