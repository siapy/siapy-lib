...
def main(cfg):
    ...
    parsed_data_out = pd.DataFrame(columns = ["images_indices", "objects_indices", "slices_indices",
                                                 "labels_all", "labels_names", "signatures", "filenames", "camera_names"])
    ...
    if match_indices:
        parsed_data_out.labels_names = parsed_data_out.apply(lambda row: parse_labels(row, ref_panel), axis=1)
